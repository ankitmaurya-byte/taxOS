...
const FOUNDER_ACTIONS: ActionItem[] = [
  { icon: FileText, title: 'What filings are due?', description: 'Check upcoming tax filing deadlines for your entities.', prompt: 'What tax filings are due for my company this quarter? List them with deadlines.', color: 'text-[#6C5CE7] bg-[#F3F0FF]' },
  ...
  { icon: DollarSign, title: 'R&D tax credits', description: 'Check eligibility for R&D tax credits.', prompt: 'Am I eligible for the R&D tax credit? What qualifying activities should I track?', color: 'text-[#6C5CE7] bg-[#F3F0FF]' },
...
const CPA_ACTIONS: ActionItem[] = [
  { icon: FileText, title: '1120 review checklist', description: 'Comprehensive checklist for corporate return review.', prompt: 'Give me a comprehensive checklist for reviewing a 1120 corporate tax return.', color: 'text-[#6C5CE7] bg-[#F3F0FF]' },
  ...
  { icon: DollarSign, title: 'Officer compensation', description: 'IRS reasonable compensation guidelines.', prompt: 'Explain the IRS reasonable compensation rules for S-Corp officers.', color: 'text-[#6C5CE7] bg-[#F3F0FF]' },
...
const ADMIN_ACTIONS: ActionItem[] = [
  { icon: Building2, title: 'Onboarding checklist', description: 'Standard flow for new founder verification.', prompt: 'What is the standard onboarding flow for new founders?', color: 'text-[#6C5CE7] bg-[#F3F0FF]' },
...
const TEAM_ACTIONS: ActionItem[] = [
  { icon: FileText, title: 'Upload a document', description: 'How to upload and tag documents.', prompt: 'Walk me through how to upload and tag a document in TaxOS.', color: 'text-[#6C5CE7] bg-[#F3F0FF]' },
      ...
      <div className="w-12 border-r border-[#E5E7EB] flex flex-col items-center py-4 gap-2 bg-[#F9FAFB]">
        <button
          ...
          className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
            activeView === 'chat' ? 'bg-[#EDE9FD] text-[#6C5CE7]' : 'text-[#9CA3AF] hover:bg-[#F3F4F6]'
          }`}
          title="New chat"
        >
          <Pencil size={16} />
        ...
        <button
          ...
          className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
            activeView === 'history' ? 'bg-[#EDE9FD] text-[#6C5CE7]' : 'text-[#9CA3AF] hover:bg-[#F3F4F6]'
          }`}
          title="History"
        >
          <History size={16} />
        ...
        <button
          ...
          className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
            activeView === 'actions' ? 'bg-[#EDE9FD] text-[#6C5CE7]' : 'text-[#9CA3AF] hover:bg-[#F3F4F6]'
          }`}
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 bg-[#FAFAFA]">
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-sm text-[#6B7280] max-w-xs mb-4">{config.empty}</p>
                  ...
                  <div className="w-full space-y-2">
                    {actions.slice(0, 4).map((action) => (
                      <button
                        ...
                        className="w-full flex items-center gap-3 rounded-xl border border-[#E5E7EB] bg-white px-3 py-2.5 text-left hover:border-[#D8D3FF] hover:shadow-sm transition-all disabled:opacity-50"
                      >
                        <div className={`flex h-8 w-8 items-center justify-center rounded-lg flex-shrink-0 ${action.color}`}>
                          <action.icon size={14} />
                        </div>
                        <div className="min-w-0">
                          <p className="text-xs font-medium text-[#111827]">{action.title}</p>
                          <p className="text-[10px] text-[#9CA3AF] truncate">{action.description}</p>
                        </div>
                      </button>
                    ))}
                  </div>
                ...
                msg.role === 'user' ? (
                  <div key={i} className="flex justify-end">
                    <div className="max-w-[280px]">
                      ...
                      <p className="text-[10px] text-[#9CA3AF] mt-1 text-right">{msg.timestamp}</p>
                    </div>
                  </div>
                ) : (
                  <div key={i} className="flex gap-2">
                    <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#6C5CE7] to-[#8B5CF6] flex items-center justify-center flex-shrink-0 mt-0.5">
                    ...
                    <div className="max-w-[280px]">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <button className="text-[#9CA3AF] hover:text-[#6B7280]"><ThumbsUp size={12} /></button>
                        <button className="text-[#9CA3AF] hover:text-[#6B7280]"><ThumbsDown size={12} /></button>
                      </div>
                    </div>
                  </div>
                ),
      ...
      <div className="space-y-2">
        {actions.map((action, i) => (
          <button
            ...
            className="w-full flex items-center gap-3 p-3.5 bg-white border border-[#E5E7EB] rounded-xl hover:border-[#6C5CE7] hover:shadow-sm transition-all text-left group"
          >
            ...
            <ChevronRight size={16} className="text-[#9CA3AF] group-hover:text-[#6C5CE7] flex-shrink-0" />
          </button>
        ))}
      </div>
    </div>
