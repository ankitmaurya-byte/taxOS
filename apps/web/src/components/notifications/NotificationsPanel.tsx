...
export function NotificationsPanel({ onClose }: NotificationsPanelProps) {
  ...
  return (
    <>
      ...
      <div className="absolute right-0 top-full z-50 mt-3 w-[460px] overflow-hidden rounded-[18px] border border-[#E5E7EB] bg-white shadow-[0_20px_50px_rgba(17,24,39,0.16)]">
        <div className="flex items-center justify-between border-b border-[#E5E7EB] px-5 py-4">
          <h3 className="text-[18px] font-semibold text-[#2F2C4A]">Notifications</h3>
          <button
            onClick={markAllRead}
            disabled={!hasNotifications}
            className="rounded-xl bg-[#F5F4F8] px-4 py-2 text-sm font-medium text-[#B8B6C8] disabled:cursor-not-allowed"
          >
            Mark all as read
          </button>
        </div>
          ...
          <div className="max-h-[520px] overflow-y-auto p-3">
            <div className="space-y-2">
              {notifications.map((item) => (
                <div
                  key={item.id}
                  className={`rounded-2xl border px-4 py-3 ${item.read ? 'border-[#F1F1F5] bg-white' : 'border-[#EEE9FF] bg-[#FCFBFF]'}`}
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span className="text-xs text-[#9CA3AF]">{formatDate(item.createdAt)}</span>
                      </div>
                      <p className="mt-1 text-sm leading-6 text-[#6B7280]">{item.message}</p>
                    </div>
                  </div>
                </div>
              ))}
