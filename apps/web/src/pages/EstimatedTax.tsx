  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-[#111827]">Estimated Tax</h1>
          <p className="mt-1 text-sm text-[#6B7280]">Track live filing activity and upcoming deadline pressure by entity.</p>
        </div>
        ...
        <div className="relative">
          <select
            ...
            className="h-9 appearance-none rounded-lg border border-[#E5E7EB] bg-white px-3 pr-8 text-sm text-[#374151] focus:outline-none focus:ring-2 focus:ring-[#6C5CE7]"
          >
      ...
      {!selectedEntity ? (
        <div className="rounded-xl border border-dashed border-[#E5E7EB] bg-white p-12 text-center">
          <p className="text-sm font-medium text-[#374151]">No entities yet</p>
          <p className="mt-1 text-sm text-[#6B7280]">Add an entity first to start tracking estimated tax obligations.</p>
        </div>
      ) : (
        <div className="space-y-8">
          <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
            <div className="rounded-2xl border border-[#E5E7EB] bg-white p-6 shadow-sm">
              <div className="mb-6 flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs uppercase tracking-[0.18em] text-[#9CA3AF]">Selected entity</p>
                  <h2 className="mt-2 text-xl font-semibold text-[#111827]">{selectedEntity.legalName}</h2>
                  <p className="mt-1 text-sm text-[#6B7280]">
                    {selectedEntity.entityType} • {selectedEntity.stateOfIncorporation} • Fiscal year end {selectedEntity.fiscalYearEnd}
                  </p>
                </div>
                <div className={`rounded-full px-3 py-1 text-xs font-medium ${statusTone.color} bg-[#F9FAFB]`}>
                  {statusTone.title}
                </div>
              </div>
              ...
              <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
                {[
                  { label: 'Projected tax', value: formatCurrency(annualProjectedTax), accent: 'text-[#6C5CE7]' },
                  ...
                  <div key={item.label} className="rounded-xl bg-[#FAFAFB] p-4">
              ...
              <div className="rounded-2xl border border-[#EEEAFB] bg-[linear-gradient(180deg,#FCFBFF_0%,#F7F4FF_100%)] p-5">
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-[#111827]">Quarterly payment schedule</p>
                    <p className="mt-1 text-sm text-[#6B7280]">Projected from live filing data for tax year {projection?.taxYear ?? currentTaxYear}.</p>
                  </div>
                ...
                <div className="flex h-40 items-end gap-3">
                  {(quarterlyPayments.length > 0 ? quarterlyPayments : timelineBars).map((payment: any) => (
                    <div key={payment.quarter || payment.id} className="flex flex-1 flex-col items-center gap-2">
                      <div className="w-full rounded-t-xl bg-[#F1ECFF] px-2 py-1 text-center text-[10px] font-medium text-[#6C5CE7]">
                        {payment.amount ? formatCurrency(payment.amount) : Math.max(payment.urgencyScore || 0, 20)}
                      </div>
                      <div
                        className={`w-full rounded-t-2xl bg-gradient-to-t ${payment.status === 'overdue' ? 'from-[#F59E0B] to-[#F9BF67]' : 'from-[#6C5CE7] to-[#A594FF]'}`}
                        style={{
                      ...
                      <div className="text-center text-[11px] text-[#6B7280]">
                        <div className="font-medium text-[#374151]">{payment.quarter || payment.formType}</div>
                        <div>{payment.dueDate ? formatDate(payment.dueDate) : 'No date'}</div>
                      </div>
                    </div>
                  ))}
                </div>
                <p className="mt-4 text-xs text-[#6B7280]">{projectionBasisLabel}</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="rounded-2xl border border-[#E5E7EB] bg-white p-6 shadow-sm">
                <div className="flex items-start gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#EEF4FF] text-[#3B82F6]">
                    <CalendarClock size={18} />
                  </div>
                  <div>
                    <h3 className="text-base font-semibold text-[#111827]">Next payment due</h3>
                    <p className="mt-1 text-sm text-[#6B7280]">{statusTone.detail}</p>
                  </div>
                </div>
                ...
                {nextQuarterPayment ? (
                  <button
                    ...
                    className="mt-4 w-full rounded-xl border border-[#E5E7EB] px-4 py-3 text-left transition-colors hover:border-[#D8D1F7] hover:bg-[#FAFAFF]"
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-[#111827]">{nextQuarterPayment.quarter} estimated payment</p>
                        <p className="mt-1 text-sm text-[#6B7280]">Due {formatDate(nextQuarterPayment.dueDate)}</p>
                      </div>
                      <div className="text-right">
                        <div className="text-sm font-semibold text-[#111827]">{formatCurrency(nextQuarterPayment.amount)}</div>
                  ...
                  <div className="mt-4 rounded-xl border border-dashed border-[#E5E7EB] bg-[#FCFCFD] p-4 text-sm text-[#6B7280]">
              ...
              <div className="rounded-2xl border border-[#E5E7EB] bg-white p-6 shadow-sm">
                <h3 className="text-base font-semibold text-[#111827]">What TaxOS is using</h3>
                <div className="mt-4 space-y-4">
                  {[
                    ...
                    <div key={index} className="flex items-start gap-3">
                      <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#EDE9FD] text-[#6C5CE7]">
                        <item.icon size={18} />
                      </div>
                      <p className="pt-1 text-sm text-[#6B7280]">{item.text}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="rounded-2xl border border-[#E5E7EB] bg-white p-6 shadow-sm">
            <div className="mb-4 flex items-center justify-between gap-4">
              <div>
                <h2 className="text-lg font-semibold text-[#111827]">Estimated-tax related activity</h2>
                <p className="mt-1 text-sm text-[#6B7280]">Live deadlines and filing records for the selected entity.</p>
              </div>
              <button
                onClick={() => navigate('/filings')}
                className="inline-flex items-center gap-1 text-sm font-medium text-[#6C5CE7] transition-colors hover:text-[#5B4BD5]"
              >
            ...
            {deadlines.length === 0 && entityFilings.length === 0 ? (
              <div className="rounded-xl border border-dashed border-[#E5E7EB] bg-[#FCFCFD] p-8 text-center">
                <p className="text-sm font-medium text-[#374151]">No activity yet for this entity</p>
                <p className="mt-1 text-sm text-[#6B7280]">Create a filing or add upcoming deadlines to populate this view.</p>
              </div>
            ) : (
              <div className="grid gap-3 md:grid-cols-2">
                {upcomingDeadlines.slice(0, 4).map((deadline: any) => (
                  <div key={deadline.id} className="rounded-xl border border-[#E5E7EB] p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-[#111827]">{deadline.formType} • {deadline.formName}</p>
                        <p className="mt-1 text-sm text-[#6B7280]">{deadline.description || 'Upcoming estimated-tax deadline'}</p>
                      </div>
          ...
          <div className="flex items-center gap-1.5 text-[12px] text-[#9CA3AF]">
            ...
            <span>
              ...
              {' '}
              <button onClick={() => navigate('/deadlines')} className="text-[#6C5CE7] hover:text-[#5B4BD5]">
                Review deadlines
              </button>
