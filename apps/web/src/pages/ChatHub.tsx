...
export function ChatHubPage() {
  ...
  return (
    <div className="h-[calc(100dvh-7rem)] flex flex-col gap-0">
      <h1 className="text-xl sm:text-2xl md:text-3xl font-normal text-[#061b31] mb-4" style={{ fontWeight: 300 }}>Chat</h1>

      {/* Tab bar */}
      <div className="flex gap-1 mb-4 bg-[#f6f9fc] p-1 rounded-md w-fit max-w-full overflow-x-auto">
        {tabs.map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
