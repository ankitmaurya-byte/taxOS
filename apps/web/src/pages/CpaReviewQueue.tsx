    ...
    <div className="space-y-6 p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#EDE9FD]">
          <ClipboardCheck size={20} className="text-[#6C5CE7]" />
        </div>
        <div>
          <h1 className="text-2xl font-semibold text-[#111827]">My Review Queue</h1>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-[#E5E7EB] py-16 text-center">
          <ClipboardCheck size={36} className="text-[#D1D5DB] mb-3" />
          <p className="text-sm font-medium text-[#374151]">No filings assigned to you yet</p>
...
function FilingCard({ filing, highlight = false }: { filing: any; highlight?: boolean }) {
  ...
  return (
    <Link
      ...
      className={`flex items-center gap-4 rounded-xl border px-5 py-4 transition-all hover:shadow-sm hover:border-[#C4B5FD] ${
        highlight ? 'border-amber-200 bg-amber-50/50' : 'border-[#E5E7EB] bg-white'
      }`}
    >
      <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#F3F4F6] flex-shrink-0">
        <FileText size={16} className="text-[#6B7280]" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="truncate text-sm font-medium text-[#111827]">{filing.formName || filing.formType}</p>
